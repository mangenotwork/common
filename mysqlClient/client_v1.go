...
func (m *Mysql) Conn() (err error) {
	m.DB, err = sql.Open("mysql", fmt.Sprintf("%s:%s@%s(%s:%s)/%s",
		m.user, m.password, "tcp", m.host, m.port, m.dataBase))

	if err != nil {
		if m.log {
			log.Error("[Sql] Conn Fail : " + err.Error())
		}
		return err
	}
...
func (m *Mysql) NewTable(table string, fields map[string]string) error {
	...
	if m.log {
		log.Info("[Sql] Exec : " + createSql.String())
		if err != nil {
			log.Error("[Sql] Error : " + err.Error())
		}
	}
...
func (m *Mysql) insert(table string, fieldData map[string]interface{}) error {
	...
	if m.log {
		log.Info("[Sql] Exec : " + insertSql.String())
		if err != nil {
			log.Error("[Sql] Error : " + err.Error())
		}
	}
...
func (m *Mysql) ToVarChar(data interface{}) string {
	...
	txt.WriteString(`"`)
	txt.WriteString(utils.AnyToString(data))
	txt.WriteString(`"`)
	return txt.String()
}
...
func dataType2Mysql(value interface{}) string {
	...
	log.Info(fmt.Sprintf("invalid sql type %s (%s)", typ.Type().Name(), typ.Kind()))
	return "text"
}
...
func (m *Mysql) ToXls(sql, outPath string) {
	...
	if err := f.SaveAs(outPath); err != nil {
		log.Error("[err] 导出失败: ", err)
		return
	}
	workPath, _ := os.Getwd()
	log.Info("[导出成功] 文件位置: ", workPath+"/"+outPath)
}
