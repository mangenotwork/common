...
func (cbc *cbcObj) Encrypt(str, key []byte) ([]byte, error) {
	...
	if err != nil {
		log.Error("[" + cbc.cryptoType + "-CBC] ERROR:" + err.Error())
		return []byte(""), err
	}
...
func (ecb *ecbObj) Encrypt(str, key []byte) ([]byte, error) {
	...
	if err != nil {
		log.Error("[" + ecb.cryptoType + "-ECB] ERROR:" + err.Error())
		return []byte(""), err
	}
	blockSize := block.BlockSize()
	if ecb.cryptoType == "aes" {
...
func (cfb *cfbObj) Encrypt(str, key []byte) ([]byte, error) {
	...
	if err != nil {
		log.Error("[" + cfb.cryptoType + "-CFB] ERROR:" + err.Error())
		return nil, err
	}
...
func (ctr *ctrObj) crypto(str, key []byte) ([]byte, error) {
	...
	if err != nil {
		log.Error("[AES-CTR] ERROR:" + err.Error())
		return []byte(""), err
	}
	//指定分组模式
